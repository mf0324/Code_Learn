 ...
 * -3：数据格式错误
 */
static int32 h264_demux_sei(H264_NALU_PACK *p_pack, uint8 *p_sei, int32 *p_sei_len)
{
	...
	*p_sei_len = 0;
	...
	//bs_reader_skip_bits(&bs, GEN_BYTE_BIT * 1);
 ...
 * -3：数据格式错误
 */
static int32 h264_demux_slice_header(H264_NALU_PACK *p_pack, uint8 *p_slice, int32 *p_slice_len)
{
	BIT_STREAM_READER bs;
	int32 read_bits;
 ...
 * other: fail
 */
static int32 h264_demux_nalu(H264_NALU_PACK* p_pack, uint8* p_buf, int32* p_buf_size)
{
	int32   left_size;
	int32   need_size;
	int32   parse_size;
	...
	*p_buf_size = 0;
	...
	/** nal_ref_idc, 2 bit */
	nal_ref_idc = (p_uint8[0] >> 5) & 0x3;

	/** nalu_type, 5 bit */
	p_pack->nalu_type = p_uint8[0] & 0x1F;
	//log_debug(_T("[h264_bs_parser::h264_demux_nalu] nalu type = %d\n"), p_pack->nalu_type);

	parse_size = left_size;
	switch( p_pack->nalu_type )
	{
	case H264_NALU_NON_IDR_SLICE:
	case H264_NALU_IDR_SLICE:
		{
			ret = h264_demux_slice_header(p_pack, p_uint8, &parse_size);
		}
		break;

	case H264_NALU_SEI:
		{
			ret = h264_demux_sei(p_pack, p_uint8, &parse_size);
		}
		break;

	case H264_NALU_SPS:
		{
			ret = h264_nalu_demux_sps(p_pack, p_uint8, &parse_size);
			if( ret == 0 )
			{
				p_pack->b_sps = 1;
			}
	...
	case H264_NALU_PPS:
		{
			ret = h264_nalu_demux_pps(p_pack, p_uint8, &parse_size);
		}
		break;
	...
	*p_buf_size = need_size + parse_size;
...
/**  
 * @brief 解析Nalu
 * @param [in] p_pack Pack句柄
 * @param [in] p_buf 输入数据地址
 * @param [in/out] p_buf_size 输入数据长度/实际解析数据长度
 * @return
 * 0：成功
 * -1：参数错误
 * -2：数据不足
 * -3：数据格式错误
 */
static int32 h264_demux_nalu2(H264_NALU_PACK *p_pack, uint8* p_buf, int32* p_buf_size)
{
	int32  left_len;
	int32  need_size;
	int32  parse_len;
	...
	*p_buf_size = 0;
	...
	/** swap endian */
	p_dest = (uint8*)&nalu_size;
	p_dest[0] = p_uint8[3];
	p_dest[1] = p_uint8[2];
	p_dest[2] = p_uint8[1];
	p_dest[3] = p_uint8[0];

	p_uint8 += H264_NALU_LENGTH_SIZE;
	left_len -= H264_NALU_LENGTH_SIZE;
	...
	//need_size = nalu_size;
	if( left_len < nalu_size )
	{
		return -2;
	}

	p_pack->p_start = p_buf;
	...
	case H264_NALU_IDR_SLICE:
		{
			ret = h264_demux_slice_header(p_pack, p_uint8, &parse_len);
		}
		break;

	case H264_NALU_SEI:
		{
			ret = h264_demux_sei(p_pack, p_uint8, &parse_len);
		}
		break;

	case H264_NALU_SPS:
		{
			ret = h264_nalu_demux_sps(p_pack, p_uint8, &parse_len);
			if( ret == 0 )
			{
				p_pack->b_sps = 1;
			}
	...
	case H264_NALU_PPS:
		{
			ret = h264_nalu_demux_pps(p_pack, p_uint8, &parse_len);
		}
		break;
...
FIN:
	...
	*p_buf_size = nalu_size + H264_NALU_LENGTH_SIZE;
...
int32 h264_nalu_pack_init(H264_NALU_PACK *p_pack)
{
	...
	memset(p_pack, 0, sizeof(H264_NALU_PACK));
...
int32 h264_nalu_find_header(H264_NALU* p_nalu, uint8* p_data, int32 data_size, int32* p_demux_size)
{
	...
	if( p_nalu == NULL || p_data == NULL || p_demux_size == NULL )
	{
		return GEN_E_INVALID_PARAM;
	}
	...
	*p_demux_size = 0;
	...
	b_find2 = 0;
	i = 0;
	while( left_size >= (H264_BS_HEADER_SIZE + H264_NALU_HEADER_SIZE) )
	{
		if( p_data[i] == 0x00 && p_data[i+1] == 0x00 )
		{
			if( p_data[i+2] == 0x01 )
			{
				/** short start code */
				...
				{
					b_find1 = 1;
					p_nalu->p_start = p_data + i;
					p_nalu->nalu_type = p_data[i+3] & 0x1F;
					p_nalu->start_code_size = 3;
					p_nalu->nalu_size = left_size;

				}else
				{
					b_find2 = 1;
					p_nalu->p_next = p_data + i;
					p_nalu->nalu_size = p_nalu->p_next - p_nalu->p_start;
					break;
				}
			...
			}else if( p_data[i+2] == 0x00 && p_data[i+3] == 0x01)
			{
				/** long start code */
				if( !b_find1 )
				{
					b_find1 = 1;
					p_nalu->p_start = p_data + i;
					p_nalu->nalu_type = p_data[i+4] & 0x1F;
					p_nalu->start_code_size = H264_BS_HEADER_SIZE;
					p_nalu->nalu_size = left_size;
...
int32 h264_nalu_demux(H264_NALU_PACK* p_pack, uint8* p_buf, int32 buf_size, int32* p_parse_size)
{
	...
	if( p_pack == NULL || p_buf == NULL || p_parse_size == NULL )
	{
		return GEN_E_INVALID_PARAM;
	}
	...
	while( left_size >= (H264_BS_HEADER_SIZE + H264_NALU_HEADER_SIZE) )
	{
		if( p_buf[i] == 0x00 && p_buf[i+1] == 0x00 )
		{
			...
			}else if( p_buf[i+2] == 0x00 && p_buf[i+3] == 0x01)
			{
				...
				i += H264_BS_HEADER_SIZE;
				left_size -= H264_BS_HEADER_SIZE;
				prefix_size = H264_BS_HEADER_SIZE;
		...
		parse_size = left_size;
		ret = h264_demux_nalu(p_pack, p_buf + i, &parse_size);
		i += parse_size;
...
int32 h264_nalu_demux_sps(H264_NALU_PACK *p_pack, uint8 *p_sps, int32 *p_sps_len)
{
	...
	*p_sps_len = 0;
	...
	if( left_len < (1 + H264_SPS_HEADER_SIZE) )
	{
		return GEN_E_NEED_MORE_DATA;
	}
